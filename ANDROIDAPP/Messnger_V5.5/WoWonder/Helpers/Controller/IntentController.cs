...
namespace WoWonder.Helpers.Controller
{
    public class IntentController
    {
        ...
        /// <summary>
        /// Open intent image and video Gallery when the request code of result is 600
        /// </summary>
        /// <param name="title"></param>
        public void OpenIntentImageAndVideoGallery(string title)
        {
            try
            {
                if (!WoWonderTools.CheckAllowedFileUpload())
                ...
                intent.SetType("*/*");
                intent.PutExtra(Intent.ExtraMimeTypes, ["image/*", "video/*"]);
                intent.PutExtra("return-data", true); //added snippet
                ...
                if (intent.ResolveActivity(Context.PackageManager) != null)
                    Context.StartActivityForResult(Intent.CreateChooser(intent, title), 600);
