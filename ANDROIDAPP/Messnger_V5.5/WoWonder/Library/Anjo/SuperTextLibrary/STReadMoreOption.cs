            ...
            try
            {
                if (TextLengthType == TypeCharacter)
                {
                    if (text.Length() <= TextLength)
                    {
                        ...
                        {
                            content = Html.FromHtml(text.ToString(), FromHtmlOptions.ModeLegacy)?.ToString();
                        }
                        else
                        {
                            // This method is deprecated but need to use for old os devices
#pragma warning disable CS0618 // Type or member is obsolete
                            content = Html.FromHtml(text.ToString())?.ToString();
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void AddReadLess(TextView textView, ICharSequence text)
        {
            try
            {
                ...
                if (Build.VERSION.SdkInt >= BuildVersionCodes.N)
                {
                    content = Html.FromHtml(ss.ToString(), FromHtmlOptions.ModeLegacy)?.ToString();
                }
                else
                {
                    // This method is deprecated but need to use for old os devices
#pragma warning disable CS0618 // Type or member is obsolete
                    content = Html.FromHtml(ss.ToString())?.ToString();
#pragma warning restore CS0618 // Type or member is obsolete
                }
