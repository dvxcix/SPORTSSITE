        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            try
            {
                if (viewHolder is SelectAccountAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        GlideImageLoader.LoadImage(ActivityContext, item.Status == "Active" ? UserDetails.Avatar : item.Avatar, holder.Image, ImageStyle.RoundedCrop, ImagePlaceholders.Drawable);
