...
namespace WoWonder.Activities.Authentication
{
    ...
    public class RegisterActivity : BaseActivity, IDialogListCallBack
    {
        ...
        private EditText EmailEditText, UsernameEditText, FirstNameEditText, LastNameEditText, PasswordEditText, PasswordRepeatEditText, GenderEditText, PhoneEditText;
                ...
                if (ChkAgree.Checked)
                {
                    if (Methods.CheckConnectivity())
                    {
                        if (!string.IsNullOrEmpty(UsernameEditText.Text.Replace(" ", "")) ||
                            ...
                            !string.IsNullOrEmpty(LastNameEditText.Text) ||
                            ...
                            !string.IsNullOrEmpty(EmailEditText.Text.Replace(" ", "")))
                        {
                            ...
                            else
                            {
                                if (PasswordRepeatEditText.Text == PasswordEditText.Text)
                                {
                                    ...
                                    var (apiStatus, respond) = await RequestsAsync.Auth.CreateAccountAsync(UsernameEditText.Text.Replace(" ", ""), FirstNameEditText.Text.Replace(" ", ""), LastNameEditText.Text.Replace(" ", ""), PasswordEditText.Text, PasswordRepeatEditText.Text, EmailEditText.Text.Replace(" ", ""), GenderStatus, PhoneEditText.Text, "", UserDetails.DeviceId);
                                    if (apiStatus == 200)
                                    {
                                        if (respond is CreatAccountObject result)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    StartActivity(AppSettings.ShowSuggestedUsersOnRegister ? new Intent(this, typeof(SuggestionsUsersActivity)) : new Intent(this, typeof(ChatTabbedMainActivity)));
                                                }
                                            }
                                        }
                                    }
