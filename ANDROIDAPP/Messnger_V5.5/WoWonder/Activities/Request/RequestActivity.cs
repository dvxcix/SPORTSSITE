                        ...
                        if (respond is GetGeneralDataObject result)
                        {
                            RunOnUiThread(() =>
                            {
                                try
                                {
                                    ...
                                    {
                                        FriendRequestTab.MAdapter.UserList = new ObservableCollection<UserDataObject>(result.FriendRequests);
                                        FriendRequestTab?.MAdapter?.NotifyDataSetChanged();

                                        ListUtils.FriendRequestsList = FriendRequestTab.MAdapter.UserList;
                                    }
                                    ...
                                    if (AppSettings.EnableChatGroup)
                                    {
                                        ...
                                        {
                                            GroupRequestTab.MAdapter.GroupList = new ObservableCollection<GroupChatRequest>(result.GroupChatRequests);
                                            GroupRequestTab?.MAdapter?.NotifyDataSetChanged();

                                            ListUtils.GroupRequestsList = GroupRequestTab.MAdapter.GroupList;
                                        }
