...
namespace WoWonder.Activities.DefaultUser
{
    ...
    public class MyProfileActivity : BaseActivity, IDialogListCallBack
    {
        ...
        private ImageView MoreButton, IconQrCode;
        ...
        private TextView TextFacebook, TextTwitter, TextInstegram, TextVk, TextYoutube;
        ...
        private void InitComponent()
        {
            try
            {
                ...
                IconQrCode = FindViewById<ImageView>(Resource.Id.IconQrCode);

                if (!AppSettings.ShowQrCodeSystem)
                    IconQrCode.Visibility = ViewStates.Gone;

                ImageUser = FindViewById<CircleImageView>(Resource.Id.imageUser);
                ...
                TextFacebook = FindViewById<TextView>(Resource.Id.textFacebook);

                TwitterButton = FindViewById<LinearLayout>(Resource.Id.TwitterLiner);
                TextTwitter = FindViewById<TextView>(Resource.Id.textTwitter);

                InstegramButton = FindViewById<LinearLayout>(Resource.Id.InstagramLiner);
                TextInstegram = FindViewById<TextView>(Resource.Id.textInstagram);

                VkButton = FindViewById<LinearLayout>(Resource.Id.VkLiner);
                TextVk = FindViewById<TextView>(Resource.Id.textVk);

                YoutubeButton = FindViewById<LinearLayout>(Resource.Id.YoutubeLiner);
                TextYoutube = FindViewById<TextView>(Resource.Id.textYoutube);
        ...
        private void AddOrRemoveEvent(bool addEvent)
        {
            try
            {
                ...
                if (addEvent)
                {
                    ...
                    IconQrCode.Click += IconQrCodeOnClick;
        ...
        private void IconQrCodeOnClick(object sender, EventArgs e)
        {
            try
            {
                Intent intent = new Intent(this, typeof(QrCodeActivity));
        ...
        private void LoadDataUser(UserDataObject data)
        {
            try
            {
                ...
                if (!WoWonderTools.StoryIsAvailable(data.UserId))
                    ImageUser.BorderColor = WoWonderTools.IsTabDark() ? Color.ParseColor("#060606") : Color.White;
                else
                    ImageUser.BorderColor = Color.ParseColor(AppSettings.MainColor);
                ...
                {
                    InstegramButton.Visibility = ViewStates.Visible;
                    TextInstegram.Text = data.Instagram;
                }
                else
                    InstegramButton.Visibility = ViewStates.Gone;

                if (!string.IsNullOrEmpty(data.Vk))
                {
                    VkButton.Visibility = ViewStates.Visible;
                    TextVk.Text = data.Vk;
                }
                else
                    VkButton.Visibility = ViewStates.Gone;

                if (!string.IsNullOrEmpty(data.Youtube))
                {
                    YoutubeButton.Visibility = ViewStates.Visible;
                ...
                if (ListUtils.SettingsSiteList?.Genders?.Count > 0)
                {
                    ...
                    {
                        case "male":
                            TextGender.Text = GetText(Resource.String.Radio_Male);
                            break;
                        case "female":
                            TextGender.Text = value ?? GetText(Resource.String.Radio_Female);
                            break;
                        default:
