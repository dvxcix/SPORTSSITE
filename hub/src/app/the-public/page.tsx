...
export default async function ThePublicPage({
  searchParams,
}: {
  ...
  return (
    <TierGate requiredTier="ultimate" label="The Public">
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{
        ...
        <Link href={`/the-public?date=${prevDate}`} style={{
        ...
        {stripDates.map(({ date: d, isSelected, isToday, dayName, dayNum }) => (
          <Link key={d} href={`/the-public?date=${d}`} style={{
        ...
        <Link href={`/the-public?date=${nextDate}`} style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          width: 36, flexShrink: 0, textDecoration: 'none',
          color: 'var(--text-3)', fontSize: 18, fontWeight: 700,
        }}>›</Link>
