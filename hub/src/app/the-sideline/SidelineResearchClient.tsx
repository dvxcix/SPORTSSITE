...
export function PlayerIdentity({ player, team }: { player: NflOddsPlayer; team?: SidelineTeam }) {
  ...
  return player.gsisId ? <Link prefetch={false} className={styles.identity} aria-label={`View ${player.name} NFL profile`} href={`/nfl/players/${encodeURIComponent(player.gsisId)}`}>{body}<span className={styles.profileArrow}>↗</span></Link> : <div className={styles.identity} title="Player profile is not linked yet">{body}</div>
}
