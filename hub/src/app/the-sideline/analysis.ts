...
function buildPlayerGameLines(rows: Row[], teams: SidelineGame['home'][]): Map<string, PlayerGameLine[]> {
  ...
  const line = (playerId: string, gameId: string) => {
    ...
    if (!games.has(gameId)) games.set(gameId, { gameId, receptions: 0, receivingYards: 0, carries: 0, rushingYards: 0, passAttempts: 0, completions: 0, passingYards: 0, passingTouchdowns: 0, scorerTouchdowns: 0, firstTouchdowns: 0 })
  ...
  for (const [gameId, scorer] of firstScores) {
    const games = byPlayer.get(scorer.playerId)
    const game = games?.get(gameId)
    if (game) game.firstTouchdowns = 1
  }
...
function buildPlayers(receiving: Row[], rushing: Row[], passing: Row[], dvpRows: Row[], pbp: Row[], directoryRows: Row[], teams: SidelineGame['home'][], headshots: Map<string, string>, weekly: Row[] = []): SidelinePlayer[] {
  ...
  const rushingIds = new Set(rushing.map(row => String(row.player_gsis_id ?? '')))
  const passingIds = new Set(passing.map(row => String(row.player_gsis_id ?? '')))

  const seedFromPlay = (idValue: unknown, nameValue: unknown, teamValue: unknown, kind: 'receiving' | 'rushing' | 'passing') => {
    const id = String(idValue ?? '')
    if (!id) return
    const bio = directory.get(id)
    ensurePlayer(playerMap, {
      player_gsis_id: id,
      player_display_name: bio?.display_name ?? nameValue,
      player_position: bio?.position,
      team_abbr: teamValue ?? bio?.latest_team,
    }, kind)
  }
  for (const row of pbp) {
    seedFromPlay(row.passer_player_id, row.passer_player_name, row.posteam, 'passing')
  ...
  for (const player of playerMap.values()) {
    ...
    if (!receivingIds.has(player.id)) {
      player.targets = pbp.filter(row => String(row.receiver_player_id ?? '') === player.id && truthy(row.pass_attempt)).length
      player.receptions = lines.reduce((sum, row) => sum + row.receptions, 0)
      player.receivingYards = lines.reduce((sum, row) => sum + row.receivingYards, 0)
      player.receivingTouchdowns = pbp.filter(row => String(row.receiver_player_id ?? '') === player.id && truthy(row.pass_touchdown)).length
      const targetPlays = pbp.filter(row => String(row.receiver_player_id ?? '') === player.id && truthy(row.pass_attempt))
      player.airYards = average(targetPlays.map(row => numeric(row.air_yards))) ?? 0
    }
    if (!rushingIds.has(player.id)) {
      player.carries = lines.reduce((sum, row) => sum + row.carries, 0)
      player.rushingYards = lines.reduce((sum, row) => sum + row.rushingYards, 0)
      player.rushingTouchdowns = pbp.filter(row => String(row.rusher_player_id ?? '') === player.id && truthy(row.rush_touchdown)).length
    }
    if (!passingIds.has(player.id)) {
      player.passAttempts = lines.reduce((sum, row) => sum + row.passAttempts, 0)
      player.completions = lines.reduce((sum, row) => sum + row.completions, 0)
      player.passingYards = lines.reduce((sum, row) => sum + row.passingYards, 0)
      player.passingTouchdowns = lines.reduce((sum, row) => sum + row.passingTouchdowns, 0)
    }
  }
  teamTargets.clear()
  teamCarries.clear()
  for (const player of playerMap.values()) {
    teamTargets.set(player.team, (teamTargets.get(player.team) ?? 0) + player.targets)
    teamCarries.set(player.team, (teamCarries.get(player.team) ?? 0) + player.carries)
  }
  ...
  for (const row of pbp) {
    ...
    if (truthy(row.pass_attempt)) {
      ...
      if (numeric(row.air_yards) >= 20) player.deepTargets += 1
      if (truthy(row.complete_pass) && yards >= 20) player.receiving20 += 1
      if (truthy(row.complete_pass) && yards >= 30) player.receiving30 += 1
      if (truthy(row.complete_pass) && yards >= 40) player.receiving40 += 1
    } else if (truthy(row.rush_attempt)) {
      const yards = numeric(row.yards_gained)
      if (yards >= 10) player.rushing10 += 1
      if (yards >= 20) player.rushing20 += 1
      if (yards >= 30) player.rushing30 += 1
      if (yards >= 40) player.rushing40 += 1
    }
  }
  ...
  return Array.from(playerMap.values())
    ...
    .map(player => {
      ...
      const touchdowns = player.position === 'QB' ? player.passingTouchdowns : player.receivingTouchdowns + player.rushingTouchdowns
      if (touchdowns > 0 || player.redZoneLooks > 0) {
        const touchdownRate = (1 - Math.exp(-(touchdowns / games))) * 100
        const recent3 = lines.length ? percent(lines.slice(0, 3).filter(item => (player.position === 'QB' ? item.passingTouchdowns : item.scorerTouchdowns) > 0).length, Math.min(3, lines.length)) : touchdownRate
        const recent5 = lines.length ? percent(lines.slice(0, 5).filter(item => (player.position === 'QB' ? item.passingTouchdowns : item.scorerTouchdowns) > 0).length, Math.min(5, lines.length)) : touchdownRate
        const categories = player.position === 'QB' ? ['passing_tds'] : player.position === 'RB' || player.position === 'FB' ? ['rushing_tds'] : ['receiving_tds']
        projections.push(makeProjection('touchdown', player.position === 'QB' ? 'Passing TD' : 'Touchdown', touchdownRate, recent3, recent5, lines.map(item => (player.position === 'QB' ? item.passingTouchdowns : item.scorerTouchdowns) > 0 ? 1 : 0), matchupAdjustment(dvp, opponent, player.position, categories), paceAdjustment, confidence, 'probability', 'chance'))
      }
      ...
      return {
        ...
        projections,
        gameLog: lines,
        receiving20: player.receiving20,
        receiving30: player.receiving30,
        receiving40: player.receiving40,
        deepTargets: player.deepTargets,
        rushing10: player.rushing10,
        rushing20: player.rushing20,
        rushing30: player.rushing30,
        rushing40: player.rushing40,
      }
    })
    .sort((a, b) => b.index - a.index)
}

function normalizedGap(row: Row) {
  const gap = nullableText(row.run_gap)?.toLowerCase()
  const location = nullableText(row.run_location)?.toLowerCase()
  if (gap && !['unknown', 'nan', 'none'].includes(gap)) return gap
  return location && !['unknown', 'nan', 'none'].includes(location) ? location : 'unspecified'
}

function buildRunGaps(rows: Row[], teams: SidelineGame['home'][]): SidelineRunGap[] {
  type Bucket = { attempts: number; yards: number; successes: number; explosives: number }
  const empty = (): Bucket => ({ attempts: 0, yards: 0, successes: 0, explosives: 0 })
  const add = (map: Map<string, Bucket>, key: string, row: Row) => {
    const bucket = map.get(key) ?? empty()
    bucket.attempts += 1
    bucket.yards += numeric(row.yards_gained)
    bucket.successes += truthy(row.success) ? 1 : 0
    bucket.explosives += numeric(row.yards_gained) >= 10 ? 1 : 0
    map.set(key, bucket)
  }
  const offense = new Map<string, Bucket>()
  const defense = new Map<string, Bucket>()
  for (const row of rows) {
    if (!truthy(row.rush_attempt)) continue
    const gap = normalizedGap(row)
    add(offense, `${text(row.posteam)}:${gap}`, row)
    add(defense, `${text(row.defteam)}:${gap}`, row)
  }
  return teams.flatMap((team, index) => {
    const opponent = teams[index === 0 ? 1 : 0]?.abbr ?? ''
    return Array.from(offense.entries())
      .filter(([key, bucket]) => key.startsWith(`${team.abbr}:`) && bucket.attempts >= 3)
      .map(([key, attack]) => {
        const gap = key.slice(team.abbr.length + 1)
        const allowance = defense.get(`${opponent}:${gap}`) ?? empty()
        const ypc = attack.yards / attack.attempts
        const defenseYpc = allowance.attempts ? allowance.yards / allowance.attempts : 0
        const success = percent(attack.successes, attack.attempts)
        const defenseSuccess = percent(allowance.successes, allowance.attempts)
        return {
          team: team.abbr,
          opponent,
          gap,
          attempts: attack.attempts,
          yardsPerCarry: round(ypc),
          successRate: success,
          explosiveRate: percent(attack.explosives, attack.attempts),
          defenseAttempts: allowance.attempts,
          defenseYardsPerCarry: round(defenseYpc),
          defenseSuccessRate: defenseSuccess,
          defenseExplosiveRate: percent(allowance.explosives, allowance.attempts),
          edge: round(((ypc - 4.2) * 8) + ((success - 42) * .55) + ((defenseYpc - 4.2) * 6) + ((defenseSuccess - 42) * .35)),
        }
      })
...
export async function getSidelineLens(game: SidelineGame, includeHistory = true, sample: NflSample = defaultNflSample(game)): Promise<SidelineLens> {
  ...
  try {
    ...
    return {
      ...
      runGaps,
      dvp,
      coverage: { pbpPlays: data.pbp.length, trackedPlayers, rosterPlayers: data.directory.length, advanced },
    }
  } catch {
    return {
      ...
      runGaps: [],
      dvp: [],
      coverage: { pbpPlays: 0, trackedPlayers: 0, rosterPlayers: 0, advanced: 'unavailable' },
    }
  }
}
