...
function ChannelSection({ title, eyebrow, channels, empty }: { title: string; eyebrow: string; channels: ChannelCardData[]; empty: React.ReactNode }) {
  return <section className="mt-9 min-w-0"><p className="text-xs font-bold uppercase tracking-widest text-zinc-500">{eyebrow}</p><div className="mb-4 mt-1 flex items-center justify-between"><h2 className="text-xl font-black text-white">{title}</h2><span className="text-xs text-zinc-600">{channels.length}</span></div>{channels.length ? <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-3 md:grid-cols-2">{channels.map(channel => <Link key={channel.id} href={`/channels/${channel.slug}`} className="group flex min-w-0 items-center gap-4 rounded-2xl border border-white/8 bg-[#101311] p-4 hover:border-lime-400/30"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/5 text-xl">{channel.icon || <Hash/>}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="truncate font-black text-white">{channel.name}</h3>{channel.channel_type !== 'public' && <LockKeyhole size={12} className="text-amber-300"/>}</div><p className="mt-1 truncate text-sm text-zinc-500">{channel.description || 'Community conversation'}</p><span className="mt-2 flex items-center gap-1 text-xs text-zinc-600"><Users size={12}/>{channel.member_count ?? 0} members</span></div><MessageSquareText size={17} className="shrink-0 text-zinc-700 group-hover:text-lime-300"/></Link>)}</div> : empty}</section>
}
