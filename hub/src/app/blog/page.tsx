...
import type { MemberRingStyle } from '@/components/social/MemberAvatar'
...
type Article = {
  ...
  author?: { username?: string | null; display_name?: string | null; avatar_url?: string | null; avatar_ring_style?: MemberRingStyle | null; avatar_ring_color?: string | null } | null
}
...
export default async function BlogPage() {
  ...
  const [{ data: { user } }, { data: blogs }] = await Promise.all([
    ...
    supabase.from('blogs').select('*, author:users(username, display_name, avatar_url, avatar_ring_style, avatar_ring_color, is_verified)').eq('status', 'published').order('created_at', { ascending: false }).limit(20),
  ])
...
function ArticleCard({ article, featured = false }: { article: Article; featured?: boolean }) {
  return <Link href={`/blog/${article.slug}`} className={`group overflow-hidden rounded-[22px] border border-white/[.08] bg-gradient-to-br from-white/[.04] to-white/[.015] transition hover:-translate-y-0.5 hover:border-lime-400/25 ${featured ? 'grid md:grid-cols-[1.05fr_.95fr]' : 'block'}`}>
    <div className={`relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(163,230,53,.17),transparent_42%),#121612] ${featured ? 'min-h-64' : 'h-40'}`}>
      {article.cover_image ? <SafeImage src={article.cover_image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" /> : <BookOpen className="absolute bottom-6 left-6 text-lime-300/40" size={featured ? 54 : 38} />}
