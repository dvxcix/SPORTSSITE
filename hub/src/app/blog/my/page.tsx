...
export default async function MyBlogsPage() {
  ...
  return <ProductPageShell narrow>
    ...
    {!blogs?.length ? <ProductPanel padded className="text-center"><FilePenLine className="mx-auto text-zinc-600" size={28} /><p className="mt-3 font-black text-white">Your library is empty</p><Link href="/blog/create" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-lime-400 px-4 py-2.5 text-xs font-black text-black"><Plus size={14} /> Start writing</Link></ProductPanel> : <div className="grid gap-3">{blogs.map((article: any) => <Link key={article.id} href={article.status === 'published' ? `/blog/${article.slug}` : `/blog/edit/${article.id}`} className="group grid grid-cols-[72px_minmax(0,1fr)_auto] items-center gap-4 rounded-2xl border border-white/[.08] bg-white/[.025] p-3 transition hover:border-lime-400/25 hover:bg-white/[.04]">
      <div className="h-16 overflow-hidden rounded-xl bg-white/[.05]">{article.cover_image ? <img src={article.cover_image} alt="" className="h-full w-full object-cover" /> : <span className="grid h-full place-items-center text-zinc-700"><BookOpen size={22} /></span>}</div>
