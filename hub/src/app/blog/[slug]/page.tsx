...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: { user } }, { data: blog }] = await Promise.all([
    ...
    supabase.from('blogs').select('*, author:users(id, username, display_name, avatar_url, avatar_ring_style, avatar_ring_color, is_verified, bio, account_type)').eq('slug', slug).eq('status', 'published').single(),
  ])
  if (!blog) notFound()
  ...
  return <ProductPageShell narrow>
    ...
    <article className="overflow-hidden rounded-[26px] border border-white/[.08] bg-gradient-to-br from-white/[.045] to-white/[.015] shadow-[0_28px_90px_rgba(0,0,0,.3)]">
      <div className="relative min-h-56 overflow-hidden bg-[radial-gradient(circle_at_20%_15%,rgba(163,230,53,.2),transparent_38%),#101510] sm:min-h-80">
        {blog.cover_image ? <SafeImage src={blog.cover_image} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <BookOpen size={64} className="absolute bottom-7 left-7 text-lime-300/30" />}
      ...
      <div className="p-5 sm:p-8">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {blog.sport && <span className="inline-flex items-center gap-1.5 rounded-full border border-white/[.07] bg-white/[.035] px-2.5 py-1 text-[9px] font-black uppercase tracking-[.14em] text-zinc-300">{sportLogoUrl(blog.sport) && <SafeImage src={sportLogoUrl(blog.sport)} alt="" className="h-3.5 w-3.5 object-contain" />}{blog.sport}</span>}
        </div>
