...
export default async function CreatorStudioPage() {
  ...
  const [{ data: profile }, { data: approval }, { data: products }, { data: groups }, { data: entitlements }, { data: events }, { data: scheduledPosts }] = await Promise.all([
  ...
  return <CreatorStudioClient profile={profile} isTestAccount={profile.username.toLowerCase() === 'slipsurge'} products={products || []} groups={groups || []} stats={{ activeMembers, revenue, offers: products?.filter(item => item.status === 'active').length ?? 0, communities: groups?.length ?? 0 }} events={events ?? []} scheduledPosts={scheduledPosts ?? []} />
}
