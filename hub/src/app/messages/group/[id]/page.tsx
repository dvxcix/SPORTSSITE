...
import { GroupDMRoom, type GroupConversationMember, type GroupMessage } from '@/components/chat/GroupDMRoom'
...
export default async function GroupMessagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <GroupDMRoom conversation={conversation} members={(members ?? []) as unknown as GroupConversationMember[]} initialMessages={(history ?? []) as unknown as GroupMessage[]} currentUserId={user.id}/>
}
