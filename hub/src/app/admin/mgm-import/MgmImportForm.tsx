...
export function MgmImportForm() {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch('/api/admin/mgm-import', {
        method: 'POST',
  ...
  return (
    <div className="space-y-4">
      ...
      <label className={`flex items-center gap-2 text-xs cursor-pointer select-none rounded-xl border px-4 py-2.5 transition-all ${isOpening ? 'border-amber-500/50 bg-amber-500/10 text-amber-400' : 'border-zinc-700 bg-zinc-800/50 text-zinc-400'}`}>
        ...
        📸 This is the opening/early paste for this game. Check this on your first paste of the day; leave unchecked for later updates.
      </label>

      <div>
        <label className="block text-xs font-bold text-zinc-400 mb-1.5">Pasted JSON from scraper (one scrape, or the whole window.__mgmAllScrapes array)</label>
        <textarea
          value={json}
          onChange={e => setJson(e.target.value)}
          rows={10}
          placeholder='{"sportsbook":"BetMGM","market":"Batter home runs","threshold":"1+","outcomes":[...]} or [ {...}, {...} ]'
      ...
      >
        {submitting ? 'Importing…' : !selectedGame ? 'Select a game first' : 'Import HR Odds'}
      </button>
    </div>
  )
}
