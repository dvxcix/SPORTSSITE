...
export default async function AdminCreatorsPage() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.section}><header><div><span>APPLICATIONS</span><h2>Creator approvals</h2></div><p>{pending.length} awaiting review · {apps.filter((item: any) => item.status === 'approved').length} approved · {apps.filter((item: any) => item.status === 'rejected').length} rejected</p></header><div className={styles.applicationGrid}>{pending.map((app: any) => <article key={app.id} className={styles.application}><div className={styles.person}><span>{app.applicant?.avatar_url ? <SafeImage src={app.applicant.avatar_url} alt="" /> : (app.applicant?.display_name || app.applicant?.username || '?')[0]}</span><div><strong>{app.applicant?.display_name || app.applicant?.username}</strong><small>@{app.applicant?.username}</small></div></div><p>{app.why_creator}</p><AdminCreatorActions applicationId={app.id} userId={app.user_id} /></article>)}{!pending.length && <p className={styles.empty}>No pending applications.</p>}</div></section>
  </div>
}
