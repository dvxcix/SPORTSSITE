...
import { CalendarDays, Clapperboard, Download, Film, LoaderCircle, Sparkles, Target } from 'lucide-react'
...
export function ContactRecapStudio() {
  ...
  return <div className="mx-auto w-full max-w-[1540px] space-y-6 p-3 pb-24 sm:p-6">
    ...
    {loading ? <div className="grid min-h-80 place-items-center rounded-3xl border border-zinc-800 bg-zinc-950"><LoaderCircle className="animate-spin text-lime-300" size={28}/></div> : error ? <div className="rounded-3xl border border-red-400/20 bg-red-400/5 p-6 text-sm text-red-300">{error}</div> : data ? <>
      <section className="space-y-3"><div className="flex flex-wrap items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl border border-lime-400/25 bg-lime-400/10 text-lime-300"><Film size={18}/></span><div><h2 className="text-xl font-black text-white">Home Run Flight</h2><p className="text-xs text-zinc-500">Every confirmed homer in slate and at-bat order.</p></div><a className="ml-auto inline-flex items-center gap-2 rounded-xl bg-lime-400 px-4 py-2.5 text-xs font-black text-black shadow-[0_0_24px_rgba(163,255,63,.18)] transition hover:bg-lime-300" href={`/api/admin/contact-recap-export?date=${date}&kind=hr`} download><Download size={15}/> Download GIF</a></div><ContactFlightStage events={data.homeRuns} title="Today's Home Runs" eyebrow="Game-by-game flight log" tone="home_run"/></section>
      <section className="space-y-3"><div className="flex flex-wrap items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl border border-orange-400/25 bg-orange-400/10 text-orange-300"><Target size={18}/></span><div><h2 className="text-xl font-black text-white">Near Home Run Flight</h2><p className="text-xs text-zinc-500">Would-have-left park outcomes, separated from confirmed home runs.</p></div><a className="ml-auto inline-flex items-center gap-2 rounded-xl border border-orange-300/35 bg-orange-400/10 px-4 py-2.5 text-xs font-black text-orange-200 transition hover:bg-orange-400/15" href={`/api/admin/contact-recap-export?date=${date}&kind=near`} download><Download size={15}/> Download GIF</a></div><ContactFlightStage events={data.nearHomeRuns} title="Today's Near Home Runs" eyebrow="Park-adjusted contact log" tone="near_hr"/></section>
