...
const resultLabels: Record<ResultFilter, string> = {
  all: 'All contact', home_run: 'Home runs', near_hr: 'Near HR', hit: 'Hits', out: 'Outs',
}
...
export function SprayChartsExplorer({ initialDate }: { initialDate: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    fetch(`/api/spray-charts?date=${encodeURIComponent(date)}`, { signal: controller.signal })
      .then(async response => {
      ...
      .then(payload => {
        ...
        setGamePk(firstWithContact?.gamePk ?? payload.games[0]?.gamePk ?? 0)
        setPlayerId(0)
        setSelectedId('')
      })
  ...
  return <main className={styles.page}>
    <header className={styles.hero}>
      ...
      <label className={styles.date}><CalendarDays size={15}/><span>Date</span><input type="date" value={date} onChange={event => setDate(event.target.value)} /></label>
    </header>
      ...
      <section className={styles.chartCard}>
        ...
        <div className={styles.workspace}>
          ...
          <aside className={styles.inspector} aria-live="polite">
            {selected ? <><div className={styles.inspectIdentity}><span style={{ '--team': getTeamColor(selected.batterTeam) } as CSSProperties}><Image src={mlbHeadshot(selected.batterId)} alt="" width={58} height={58}/></span><div><small>{eventLabel(selected)}</small><h3>{selected.batterName}</h3><p>{selected.batterTeam} · off {selected.pitcherName}</p></div></div><div className={styles.inspectMetrics}><span><small>Exit velocity</small><b>{fmt(selected.exitVelocity, ' mph')}</b></span><span><small>Distance</small><b>{fmt(selected.distance, ' ft')}</b></span><span><small>Launch angle</small><b>{fmt(selected.launchAngle, '°')}</b></span><span><small>Game moment</small><b>{selected.half} {selected.inning ?? '-'}</b></span></div>{selected.coordinateSource === 'bearing_projection' ? <p className={styles.disclosure}>Landing point projected from the recorded distance and bearing.</p> : <p className={styles.official}>Official Statcast landing coordinate</p>}</> : <p>Select a marker to inspect the contact.</p>}
          </aside>
        </div>
        <footer><span><i style={{ background: '#a3ff3f' }}/> Home run</span><span><i style={{ background: '#ff9f43' }}/> Near HR</span><span><i style={{ background: '#38d9ff' }}/> Hit</span><span><i style={{ background: '#8b96aa' }}/> Out</span></footer>
      </section>
