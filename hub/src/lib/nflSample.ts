 ...
 * prior regular season through Week 1. Once every team has a current-season
 * game, Week 2 onward defaults to the live regular-season sample.
 */
export function defaultNflSample(game: { gameType: string; week: number }): NflSample {
  if (game.gameType === 'PRE') return 'preseason'
  return game.gameType === 'REG' && game.week > 1 ? 'regular' : 'previous'
}
