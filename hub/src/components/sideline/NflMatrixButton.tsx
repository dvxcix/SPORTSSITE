  ...
  return (
    <article className={styles.factor}>
      <div className={styles.factorTop}>
        {pipeline ? <select aria-label="Pipeline step type" value={step.kind} onChange={event => onChange({ ...step, kind: event.target.value as 'filter' | 'rank' })}><option value="filter">Filter</option><option value="rank">Rank</option></select> : null}
        <select aria-label="Condition category" value={factor.category} onChange={event => setCategory(event.target.value as NflMatrixCategory)}>
      ...
      {factor.category === 'market' ? (
        <div className={styles.factorGrid}>
          <select aria-label="Sportsbook market" value={factor.propType ?? 'anytime_td'} onChange={event => onChange({ ...factor, propType: event.target.value })}>{NFL_MATRIX_PROP_TYPES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
          <select aria-label="Sportsbook" value={factor.vendor ?? 'fanduel'} onChange={event => onChange({ ...factor, vendor: event.target.value })}>{NFL_MATRIX_BOOKS.map(book => <option value={book} key={book}>{book}</option>)}</select>
          <select aria-label="Market value" value={factor.marketValue ?? 'current'} onChange={event => onChange({ ...factor, marketValue: event.target.value as NflMatrixFactor['marketValue'] })}><option value="current">Current odds</option><option value="opening">Opening odds</option><option value="move">Current - open odds</option><option value="line">Prop line</option><option value="probability_move">Implied probability change (pp)</option><option value="ratio">ATD / contract ratio</option><option value="ratio_move">ATD / contract ratio change</option><option value="estimated_picks">Estimated OVER picks · model v1</option></select>
        </div>
      ) : factor.category === 'picks' ? (
        <div className={styles.factorGrid}>
          <select aria-label="Public pick market" value={factor.propType ?? 'anytime_td'} onChange={event => onChange({ ...factor, propType: event.target.value })}>{NFL_MATRIX_PROP_TYPES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
          <span className={styles.fixedContext}>Observed picks · latest board</span>
        </div>
      ) : (
        <div className={styles.factorGrid}>
          <select aria-label="Condition field" value={factor.field} onChange={event => onChange({ ...factor, field: event.target.value })}>{fields.map(field => <option value={field.field} key={field.field}>{field.label}</option>)}</select>
          {!['team', 'baseline'].includes(factor.category) ? <select aria-label="Stat window" value={factor.window} onChange={event => onChange({ ...factor, window: event.target.value as NflMatrixFactor['window'] })}>{WINDOWS.map(window => <option value={window} key={window}>{window === 'season' ? 'Season' : window.toUpperCase()}</option>)}</select> : <span className={styles.fixedContext}>Current game context</span>}
        </div>
      )}
      ...
      {pipeline && step.kind === 'rank' ? (
        <div className={styles.conditionRow}><span>Keep</span><input aria-label="Number of ranked players to keep" type="number" min={1} max={20} value={step.keep} onChange={event => onChange({ ...step, keep: Number(event.target.value) })} /><select aria-label="Rank direction" value={step.direction} onChange={event => onChange({ ...step, direction: event.target.value as 'highest' | 'lowest' })}><option value="highest">highest</option><option value="lowest">lowest</option></select><select aria-label="Rank scope" value={step.scope} onChange={event => onChange({ ...step, scope: event.target.value as 'team' | 'game' })}><option value="team">per team</option><option value="game">in game</option></select></div>
      ) : (
        <div className={styles.conditionRow}>
          {pipeline ? <select aria-label="Condition join" value={step.join} onChange={event => onChange({ ...step, join: event.target.value as 'and' | 'or' })}><option value="and">AND</option><option value="or">OR</option></select> : null}
          <select aria-label="Condition operator" value={factor.operator} onChange={event => onChange({ ...factor, operator: event.target.value as NflMatrixFactor['operator'] })}><option value="gte">At least</option><option value="lte">At most</option><option value="eq">Exactly</option><option value="up">Moved up</option><option value="down">Moved down</option><option value="flat">Stayed flat</option><option value="is_available">Is posted</option></select>
          {['gte', 'lte', 'eq'].includes(factor.operator) ? <input aria-label="Condition value" type="number" step="any" value={factor.value ?? ''} onChange={event => onChange({ ...factor, value: event.target.value === '' ? null : Number(event.target.value) })} placeholder="Value" /> : null}
        </div>
      )}
    </article>
  )
