  ...
  return <div ref={shellRef} className={styles.chartShell} onMouseLeave={() => setCursor(null)} onMouseMove={event => {
    ...
    {cursor && typeof document !== 'undefined' && createPortal(<div className={styles.chartTooltip} data-side={cursor.clientX > window.innerWidth * .62 ? 'left' : 'right'} style={{ left: cursor.clientX, top: Math.max(76, Math.min(window.innerHeight - 300, cursor.clientY)) }}>
      ...
      {cursorEntries.slice(0, 4).map((s, index) => { const step=s.previous?s.point.odds-s.previous.odds:0;const fromOpen=s.point.odds-s.open;return <div className={styles.inspectCard} data-focus={index===0} key={s.id} style={{'--signal':s.color} as CSSProperties}>
        <span className={styles.inspectAvatar}><SafeImage src={mlbHeadshot(s.playerId)} alt=""/><SafeImage src={getTeamLogoUrl(s.team)} alt=""/></span>
  ...
  return <div className={`${styles.page} ${embedded ? styles.embedded : ''}`}>
    ...
    {!embedded && <section className={styles.gameRail} aria-label="Select game">
      {loading ? Array.from({length:6},(_,i)=><div className={styles.gameSkeleton} key={i}/>) : games.map(g => <button key={g.gamePk} data-active={g.gamePk===gamePk} onClick={() => chooseGame(g.gamePk)} aria-label={`${g.awayAbbr} at ${g.homeAbbr}`}>
        <span style={{'--team':getTeamColor(g.awayAbbr)} as CSSProperties}><SafeImage src={getTeamLogoUrl(g.awayAbbr)} alt=""/></span><b>VS</b><span style={{'--team':getTeamColor(g.homeAbbr)} as CSSProperties}><SafeImage src={getTeamLogoUrl(g.homeAbbr)} alt=""/></span><small>{g.status === 'Final' ? 'FINAL' : new Date(g.gameDate).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</small>
      </button>)}
    </section>}

    {game && <>
      <section className={styles.matchupHeader}>
        <div><span className={styles.pitcherAvatar} style={{'--team':getTeamColor(game.awayAbbr)} as CSSProperties}>{game.awayPitcher?.id ? <SafeImage src={mlbHeadshot(game.awayPitcher.id)} alt=""/> : null}<SafeImage className={styles.pitcherTeamLogo} src={getTeamLogoUrl(game.awayAbbr)} alt=""/></span><div><small>{historical ? 'FINAL STARTER' : 'PROJECTED STARTER'}</small><strong>{game.awayPitcher?.name ?? 'TBD'}</strong><span>{game.awayPitcher?.hand ? `${game.awayPitcher.hand}HP` : 'Hand TBD'}</span></div></div>
        <div className={styles.matchupPulse}><span/><b>{captureCount.toLocaleString()}</b><small>CAPTURES</small></div>
        <div><div><small>{historical ? 'FINAL STARTER' : 'PROJECTED STARTER'}</small><strong>{game.homePitcher?.name ?? 'TBD'}</strong><span>{game.homePitcher?.hand ? `${game.homePitcher.hand}HP` : 'Hand TBD'}</span></div><span className={styles.pitcherAvatar} style={{'--team':getTeamColor(game.homeAbbr)} as CSSProperties}>{game.homePitcher?.id ? <SafeImage src={mlbHeadshot(game.homePitcher.id)} alt=""/> : null}<SafeImage className={styles.pitcherTeamLogo} src={getTeamLogoUrl(game.homeAbbr)} alt=""/></span></div>
      </section>
      ...
      <section className={styles.terminal}>
        ...
        {focusedSeries.length>0&&<div className={styles.focusBar}><span>FOCUSED LINES</span><div>{focusedSeries.map(item=><button type="button" key={item.id} style={{'--signal':item.color} as CSSProperties} onClick={()=>toggleLineFocus(item.id)}><SafeImage src={mlbHeadshot(item.playerId)} alt=""/><BookLogo vendor={item.book} size={11}/><strong>{item.name}</strong><small>{item.bookLabel}</small>{pinnedLineIds.has(item.id)&&<Pin size={10}/>}</button>)}</div><button type="button" onClick={()=>{setActiveLineId(null);setPinnedLineIds(new Set())}}>SHOW ALL</button></div>}
