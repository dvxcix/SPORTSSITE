...
export function AccountDeletionControl({ hasPaidTier }: { hasPaidTier: boolean }) {
  ...
  if (current) return (
    <div className="rounded-xl border border-amber-500/25 bg-amber-500/10 p-4">
      ...
      {current.status !== 'scheduled' ? <button type="button" onClick={cancel} disabled={submitting} className="mt-3 flex items-center gap-2 rounded-lg border border-amber-400/30 px-3 py-2 text-xs font-black text-amber-200 hover:bg-amber-400/10 disabled:opacity-50"><X size={13} /> Cancel request</button> : null}
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea aria-label="Reason for leaving" value={reason} onChange={event => setReason(event.target.value)} maxLength={1000} placeholder="Optional: tell us why you are leaving" className="ss-input min-h-20 resize-y text-sm" />
      ...
      <button type="button" onClick={submit} disabled={submitting || confirmation !== 'DELETE'} className="rounded-xl bg-red-500 px-4 py-2.5 text-sm font-black text-black transition-colors hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-40">{submitting ? 'Submitting…' : 'Request account deletion'}</button>
