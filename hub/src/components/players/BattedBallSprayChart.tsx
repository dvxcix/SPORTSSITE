...
export function BattedBallSprayChart({ rows, playerName }: { rows: SprayPitchRow[]; playerName: string }) {
  ...
  const currentPark = parks.find(park => park.key === parkKey) ?? parks[0]
  const parkRows = useMemo(() => eligible.filter(row => rowPark(row).key === currentPark?.key), [currentPark?.key, eligible])
  ...
  const allowedGames = useMemo(() => new Set(windowKey === 'season' ? gameKeys : gameKeys.slice(0, Number(windowKey))), [gameKeys, windowKey])
  ...
  const visibleRows = useMemo(() => parkRows.filter(row => {
    ...
    if (throwHand !== 'all' && row.p_throws !== throwHand) return false
    if (pitchType !== 'all' && row.pitch_type !== pitchType) return false
    return true
  }), [allowedGames, parkRows, pitchType, resultKey, throwHand])
  ...
  return (
    <section className={styles.card} aria-labelledby="spray-title">
      <header className={styles.header}>
        <div className={styles.titleBlock}>
          <span className={styles.icon}><Crosshair size={20} /></span>
          <div>
            <p className={styles.eyebrow}>Park-specific contact map</p>
            <h2 id="spray-title">Batted Ball Spray</h2>
            <p>Every marker uses the official venue, Statcast landing coordinate, and scored result.</p>
          </div>
        </div>
        <div className={styles.summary}>
          <span><strong>{totals.bbe}</strong> BBE</span>
          <span><strong>{totals.hr}</strong> HR</span>
          <span><strong>{totals.near}</strong> near HR</span>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.segment} aria-label="Game window">
      ...
      <div className={styles.filters}>
        <span className={styles.filterLabel}><SlidersHorizontal size={14} /> Filters</span>
        {(['all', 'home_run', 'near_hr', 'hits', 'outs'] as ResultKey[]).map(value => (
          <button key={value} type="button" className={resultKey === value ? styles.filterActive : ''} onClick={() => setResultKey(value)}>
            {{ all: 'All contact', home_run: 'Home runs', near_hr: 'Near HR', hits: 'Hits', outs: 'Outs' }[value]}
          </button>
        ))}
        ...
        <select aria-label="Pitch type" value={pitchType} onChange={event => setPitchType(event.target.value)}>
          ...
          {pitchTypes.map(value => <option key={value} value={value}>{value}</option>)}
        </select>
      ...
      <div className={styles.workspace}>
        <div className={styles.stage}>
          <div className={styles.parkLabel}>
            ...
            <span><strong>{currentPark.name}</strong>{currentPark.exact ? 'Traced park geometry' : 'Official venue · neutral-site outline unavailable'}</span>
          </div>
          ...
          <ParkFieldSvg
            ...
            ariaLabel={`${playerName} batted-ball spray chart at ${currentPark.name}`}
          >
            <defs>
            ...
            {visibleRows.map((row, index) => {
              ...
              return (
                <g
                  ...
                  onMouseEnter={() => setSelectedKey(rowKey)}
          ...
          {selected ? (
            <>
              <div className={styles.inspectorTop}>
                ...
                <div><p>{selected.is_near_hr && !selected.is_home_run ? 'Near-home-run contact' : eventLabel(selected.events)}</p><span>{formatDate(selected.game_date)} · {currentPark.name}</span></div>
              </div>
      ...
      <footer className={styles.footer}>
        ...
        <p>Official Statcast coordinates and scoring. Park outlines are visual context and never determine the result.</p>
      </footer>
    </section>
  )
}
