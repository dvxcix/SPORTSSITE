...
type Workspace = { id: string; name: string; sport: string | null; watchlist_item_ids: string[]; mlb_matrix_ids: string[]; nfl_matrix_ids: string[]; created_at: string; updated_at: string }
...
export function ResearchWorkspaceClient({ userId, initialItems, initialMlbMatrices, initialNflMatrices, initialWorkspaces, initialNotes }: {
  ...
  async function createWorkspace() {
    ...
    const { data, error } = await db.from('research_workspaces').insert({ user_id: userId, name, sport: 'MULTI', watchlist_item_ids: draftIds, mlb_matrix_ids: mlbMatrixIds, nfl_matrix_ids: nflMatrixIds }).select('id,name,sport,watchlist_item_ids,mlb_matrix_ids,nfl_matrix_ids,created_at,updated_at').single()
    setBusy('')
  ...
  async function saveWorkspace() {
    ...
    const { error } = await db.from('research_workspaces').update({ watchlist_item_ids: draftIds, mlb_matrix_ids: mlbMatrixIds, nfl_matrix_ids: nflMatrixIds }).eq('id', active.id).eq('user_id', userId)
    setBusy('')
  ...
  return <div className={styles.layout}>
    <aside className={styles.sidebar}>
      ...
      <div className={styles.workspaceList}>{workspaces.map(workspace => <button type="button" key={workspace.id} className={workspace.id === activeId ? styles.workspaceActive : styles.workspaceButton} onClick={() => chooseWorkspace(workspace)}><span>{workspace.name}</span><small>{workspace.watchlist_item_ids.length} compared</small></button>)}{!workspaces.length && <div className={styles.emptyMini}>Build your first saved research view.</div>}</div>
    ...
    <div className={styles.main}>
      <section className={styles.comparePanel}>
        <div className={styles.panelHeader}><div><span className={styles.eyebrow}>Up to four saved markets</span><h2>Compare Tray</h2></div><div className={styles.headerActions}><span>{compared.length}/4</span><button type="button" onClick={() => void saveWorkspace()} disabled={busy === 'save'}>{busy === 'save' ? <LoaderCircle className={styles.spin} size={15}/> : <Save size={15}/>}Save</button>{active && <button className={styles.dangerButton} type="button" onClick={() => void deleteWorkspace()} disabled={busy === 'delete'} aria-label="Delete workspace"><Trash2 size={15}/></button>}</div></div>
