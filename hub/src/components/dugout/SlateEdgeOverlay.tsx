  ...
  const captureEvidence = (entry: SlateEdgeEntry, rank: number, edge: MlbSlateEdgeRank | undefined) => {
    ...
    const evidence: SlateEdgeEvidence = {
      ...
      snapshot: {
        ...
        score: entry.score,
        scoreConfidence: entry.scoreConfidence,
        modelRank: entry.modelRank,
        bookRank: entry.bookRank,
        mm: entry.mm,
        pitchFit: entry.pitchFit,
        barrelRecent: entry.barrelRecent,
        barrelDelta: entry.barrelDelta,
        hardHitDelta: entry.hardHitDelta,
        pullAirRecent: entry.pullAirRecent,
        pullAirDelta: entry.pullAirDelta,
        timingDelta: entry.timingDelta,
        tags: evidenceTagsFor(entry).map(tag => tag.label),
        hr: entry.hr,
        hrOpen: entry.hrOpen,
        hrBaseline: entry.hrBaseline,
        fhr: entry.fhr,
        fhrOpen: entry.fhrOpen,
        fhrBaseline: entry.fhrBaseline,
        hrBooks: entry.hrBooks,
        marketLadder: entry.marketLadder,
        publicPicks: entry.publicPicks,
      },
    }
