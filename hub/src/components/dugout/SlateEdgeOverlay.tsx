...
import { Activity, BarChart3, Crosshair, FolderOpen, Search, X } from 'lucide-react'
...
type View = 'rankings' | 'vault' | 'market' | 'signals'
      ...
      <div className={styles.shell}>
        <header className={styles.header}>
          ...
          <div className={styles.headerMeta}>
            ...
            <span className={styles.metaPill}>Window follows Dugout</span>
        ...
        <section className={styles.controlDeck} aria-label="Slate Edge controls">
          ...
          <div className={styles.filters}>
            ...
            <div className={styles.gameRail} aria-label="Filter by game">
              <button type="button" className={styles.gameChip} data-active={game === 'all'} onClick={() => setGame('all')}>Full slate</button>
              {games.map(item => <button key={item.gameKey} type="button" className={styles.gameChip} data-active={game === item.gameKey} onClick={() => setGame(item.gameKey)}><TeamLogo abbr={item.awayAbbr} />{item.awayAbbr}<span>at</span><TeamLogo abbr={item.homeAbbr} />{item.homeAbbr}</button>)}
            </div>
        ...
        <main className={styles.content}>
          <section className={styles.summary} aria-label="Slate summary">
            <div className={styles.summaryCard}><small>Board leader</small><strong>{leader?.name ?? '—'}</strong><em>{leader?.score != null ? `Score ${Math.round(leader.score)}` : 'No score'}</em></div>
          ...
          {view === 'rankings' && (filtered.length ? (
            <div className={styles.tableWrap}>
              <table className={styles.table}>
                <thead><tr><th>Rank</th><th>Player</th><th>Game</th><th>Score</th><th>Pitch fit</th><th>Recent barrel</th><th>FD FHR</th><th>FD HR</th><th>Picks</th><th>MM</th></tr></thead>
                <tbody>{filtered.map((entry, index) => {
                  ...
                  return <tr key={`${entry.gameKey}:${entry.mlbId ?? entry.name}`} onClick={() => openPlayer(entry)}>
                    ...
                    <td><div className={styles.player}><Avatar entry={entry} /><span className={styles.playerCopy}><strong>{entry.name}</strong><span>{entry.team} · {entry.position}{entry.battingOrder ? ` · #${entry.battingOrder}` : ''}</span></span></div></td>
                    ...
                    <td className={styles.score}>{entry.score != null ? Math.round(entry.score) : '—'}</td>
                    <td>{entry.pitchFit != null ? Math.round(entry.pitchFit) : '—'}</td>
                    <td>{pctRaw(entry.barrelRecent)}{entry.barrelDelta != null && <span className={`${styles.movement} ${entry.barrelDelta > 0 ? styles.positive : entry.barrelDelta < 0 ? styles.negative : styles.muted}`}>{signed(entry.barrelDelta)}</span>}</td>
                    <td className={styles.odds}>{odds(entry.fhr)}{fhrMove != null && <span className={`${styles.movement} ${movementClass(fhrMove)}`}>{signed(fhrMove)}</span>}</td>
                    <td className={styles.odds}>{odds(entry.hr)}{hrMove != null && <span className={`${styles.movement} ${movementClass(hrMove)}`}>{signed(hrMove)}</span>}</td>
                    <td className={styles.pickCount}>{entry.publicPicks?.toLocaleString() ?? '—'}</td>
                    <td className={(entry.mm ?? 0) > 0 ? styles.positive : (entry.mm ?? 0) < 0 ? styles.negative : styles.muted}>{signed(entry.mm)}</td>
                  </tr>
                })}</tbody>
              </table>
            </div>
          ...
          {view === 'vault' && <div className={styles.gameGrid}>{games.filter(item => game === 'all' || item.gameKey === game).map(gameEntry => {
            ...
            return <article className={styles.gameCard} key={gameEntry.gameKey}>
              ...
              <div className={styles.gamePlayers}>{players.map((entry, index) => <button type="button" className={styles.gamePlayer} key={`${entry.gameKey}:${entry.mlbId ?? entry.name}`} onClick={() => openPlayer(entry)}><Avatar entry={entry} size={28} /><b>{index + 1}. {entry.name}</b><span className={styles.score}>{entry.score != null ? Math.round(entry.score) : '—'}</span><span>{odds(entry.hr)}</span><span className={(entry.mm ?? 0) > 0 ? styles.positive : (entry.mm ?? 0) < 0 ? styles.negative : styles.muted}>MM {signed(entry.mm)}</span></button>)}</div>
            </article>
          })}</div>}
          ...
          {view === 'market' && <div className={styles.split}>
            {[[modelAheadRows, 'Model ahead', 'Paper rank leads book rank'], [marketAheadRows, 'Market ahead', 'Book rank leads paper rank']] .map(([rows, label, note]) => <section className={styles.lane} key={String(label)}>
              ...
              {(rows as SlateEdgeEntry[]).slice(0, 18).map(entry => <button type="button" className={styles.mismatchRow} key={`${entry.gameKey}:${entry.mlbId ?? entry.name}`} onClick={() => openPlayer(entry)}><Avatar entry={entry} size={32} /><span className={styles.mismatchCopy}><strong>{entry.name}</strong><span>{entry.gameLabel} · HR {odds(entry.hr)} · {entry.publicPicks?.toLocaleString() ?? 0} picks</span></span><span className={styles.rankPair}><small>Model</small><small>Book</small><b>#{entry.modelRank ?? '—'}</b><b>#{entry.bookRank ?? '—'}</b></span></button>)}
          ...
          {view === 'signals' && (signals.length ? <div className={styles.signalGrid}>{signals.map(({ entry, chips }) => <article role="button" tabIndex={0} className={styles.signalCard} key={`${entry.gameKey}:${entry.mlbId ?? entry.name}`} onClick={() => openPlayer(entry)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') openPlayer(entry) }}>
            <div className={styles.signalTop}><Avatar entry={entry} size={34} /><span><strong>{entry.name}</strong><small>{entry.gameLabel} · {entry.team}</small></span><b className={styles.signalScore}>{entry.score != null ? Math.round(entry.score) : '—'}</b></div>
