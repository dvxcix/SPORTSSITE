...
create policy "Can send accessible message"
  ...
  with check (
    ...
    and (
      (channel_id is not null and dm_recipient_id is null and private.can_access_channel(channel_id, (select auth.uid())))
      or
...
declare
  ...
  created_id uuid;
begin
  if actor_id is null or recipient_id is null or actor_id = recipient_id then raise exception 'invalid forward target'; end if;
  ...
  if source_message.sender_id <> actor_id and source_message.dm_recipient_id <> actor_id
    and not (source_message.channel_id is not null and exists (select 1 from public.channel_members cm where cm.channel_id = source_message.channel_id and cm.user_id = actor_id))
  then raise exception 'message unavailable'; end if;
  if not exists (select 1 from public.users recipient where recipient.id = recipient_id and recipient.allow_dms = true) then raise exception 'message unavailable'; end if;
  if exists (select 1 from public.blocks b where (b.blocker_id = actor_id and b.blocked_id = recipient_id) or (b.blocker_id = recipient_id and b.blocked_id = actor_id)) then raise exception 'message unavailable'; end if;
