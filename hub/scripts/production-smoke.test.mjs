...
test('activity and account menus preserve explicit unread state and complete navigation', async () => {
  ...
  assert.match(topbar, /<FloatingSurface[\s\S]{0,420}mobileSheet/, 'mobile notifications must escape the sticky header through a portaled sheet')
  assert.match(topbar, /ref=\{notifTriggerRef\}/, 'notification sheet needs a stable trigger anchor')
  assert.ok(!topbar.includes('notifRef.current.contains'), 'a header-scoped outside-click handler must not close a portaled notification sheet')
  assert.ok(topbar.includes('aria-controls="topbar-notification-center"'))
  assert.match(topbar, /<FloatingSurface[\s\S]{0,420}className="ss-dropdown ss-topbar-profile-menu"[\s\S]{0,220}mobileSheet/, 'account controls must use the same portaled mobile-sheet system')
...
test('group settings synchronize workspace identity atomically', async () => {
  ...
  assert.ok(settings.includes("uploadMedia(file, kind === 'avatar' ? 'avatars' : 'banners')"))
  assert.ok(!settings.includes('setError(err.message)'))
...
test('member interests persist across onboarding and remain privately editable', async () => {
  ...
  assert.ok(onboardingSave.includes('interest_settings: {'))
...
test('authentication and onboarding recover without exposing provider failures', async () => {
  const login = await read('src/app/auth/login/page.tsx')
  const register = await read('src/app/auth/register/page.tsx')
  const registrationErrors = await read('src/lib/authRegistration.ts')
  ...
  assert.ok(register.includes(".eq('username', normalizedUsername)"))
  assert.ok(register.includes('registrationErrorMessage(signUpError)'))
  assert.ok(!register.includes('setError(signUpError.message)'))
  assert.ok(!register.includes('Could not create your account. Check your details and try again.'))
  assert.ok(registrationErrors.includes("code.includes('over_email_send_rate_limit')"))
  assert.ok(registrationErrors.includes("code.includes('user_already_exists')"))
  assert.ok(registrationErrors.includes("error.name === 'AuthRetryableFetchError'"))
  assert.ok(registrationErrors.includes('value.trim().toLowerCase()'))
  ...
  assert.ok(onboardingSave.includes('supabase.auth.getUser()'))
...
test('Dugout Market Story restores an exact validated capture', async () => {
  const dugout = await read('src/components/dugout/DugoutClient.tsx')
  ...
  assert.match(dugout, /aria-label="Market history"/)
})
...
test('Community groups provide secure multi-channel workspaces and staff-only announcements', async () => {
  ...
  assert.match(groupPage, /rpc\('get_my_channel_permissions'/)
  assert.match(groupPage, /readOnly=\{!canSendChannel\}/)
  assert.match(chat, /Only community staff can post in this channel/)
})

test('Community roles are durable, scoped, assignable, and channel-aware', async () => {
  const migration = await read('supabase/migrations/20260913006000_community_custom_roles_permissions.sql')
  const manager = await read('src/components/groups/GroupRoleManager.tsx')
  const settings = await read('src/app/groups/[slug]/settings/page.tsx')
  const create = await read('src/components/groups/CreateGroupForm.tsx')
  const appearance = await read('src/components/groups/GroupSettingsForm.tsx')
  const invite = await read('src/components/groups/GroupInviteModal.tsx')
  const groupPage = await read('src/app/groups/[slug]/page.tsx')
  const createPage = await read('src/app/groups/create/page.tsx')
  assert.match(migration, /alter table public\.community_roles enable row level security/)
  assert.match(migration, /foreign key \(group_id, user_id\) references public\.group_members/)
  assert.match(migration, /private\.community_channel_permission/)
  assert.match(migration, /create_community_role/)
  assert.match(migration, /set_community_member_role/)
  assert.match(migration, /set_channel_role_override/)
  assert.match(migration, /can_manage_community_roles/)
  assert.match(migration, /create or replace function public\.invite_community_member/)
  assert.match(migration, /private\.community_has_permission\(p_group_id,auth\.uid\(\),'create_invites'\)/)
  assert.match(migration, /create or replace function private\.can_manage_group_channels/)
  assert.match(migration, /if v_user_id is null then raise exception/)
  assert.match(migration, /p_creator_product_id is not null and not exists/)
  assert.doesNotMatch(createPage, /hasCreatorAccess|creators\/apply/)
  assert.match(manager, /rpc\('create_community_role'/)
  assert.match(manager, /rpc\('set_community_member_role'/)
  assert.match(manager, /value="inherit"/)
  assert.match(settings, /<GroupRoleManager/)
  assert.match(settings, /canManageChannels/)
  assert.match(settings, /canManageRoles/)
  assert.match(invite, /rpc\('invite_community_member'/)
  assert.match(groupPage, /canInvite/)
  assert.match(create, /uploadMedia\(avatarFile, 'avatars'\)/)
  assert.match(create, /uploadMedia\(bannerFile, 'banners'\)/)
  assert.match(appearance, /selectImage\('avatar'/)
  assert.match(appearance, /selectImage\('banner'/)
})
...
test('product telemetry is privacy-minimal, bounded, admin-only, and retained for 90 days', async () => {
  const migration = await read('supabase/migrations/20260913005000_product_interaction_telemetry.sql')
  const route = await read('src/app/api/telemetry/product/route.ts')
  const recorder = await read('src/lib/productTelemetry.ts')
  const observer = await read('src/components/layout/ProductTelemetry.tsx')
  const shell = await read('src/components/layout/RootLayoutShell.tsx')
  const auditPage = await read('src/app/admin/product-audit/page.tsx')
  const retention = await read('src/app/api/cron/prune-notifications/route.ts')
  assert.match(migration, /alter table public\.product_interaction_events enable row level security/)
  assert.match(migration, /member\.account_type = 'admin'/)
  assert.match(migration, /revoke all on public\.product_interaction_events from public, anon, authenticated/)
  assert.match(migration, /v_rate_key text := 'product-telemetry:' \|\| p_session_id::text/)
  assert.match(migration, /if v_count > 120 then return false/)
  assert.doesNotMatch(migration, /metadata|content|query_text|provider/)
  assert.match(route, /new URL\(origin\)\.host !== request\.nextUrl\.host/)
  assert.match(route, /durationMs > 120_000/)
  assert.match(recorder, /keepalive: true/)
  assert.match(recorder, /now - \(recent\.get\(key\)/)
  assert.match(observer, /durationMs > 2_500 \? 'slow_interaction' : 'route_ready'/)
  assert.match(shell, /<ProductTelemetry \/>/)
  assert.match(auditPage, /from\('product_interaction_events'\)/)
  assert.match(auditPage, /\.limit\(5000\)/)
  assert.match(retention, /productTelemetryCutoff = new Date\(now - 90/)
  assert.match(retention, /from\('product_interaction_events'\)\.delete/)
})

test('feature rollouts are private, deterministic, staged, and evaluated without exposing configuration', async () => {
  const migration = await read('supabase/migrations/20260912201305_staged_feature_rollouts.sql')
  const server = await read('src/lib/featureFlags.server.ts')
  const publicRoute = await read('src/app/api/features/route.ts')
  const adminRoute = await read('src/app/api/admin/feature-rollouts/route.ts')
  const manager = await read('src/app/admin/settings/features/FeatureRolloutManager.tsx')
  assert.match(migration, /rollout_percent between 0 and 100/)
  assert.match(migration, /audience in \('off', 'admins', 'members', 'percentage', 'everyone'\)/)
  assert.match(migration, /alter table public\.feature_rollouts enable row level security/)
  assert.match(migration, /alter table public\.feature_rollout_overrides enable row level security/)
  assert.match(migration, /revoke all on public\.feature_rollouts from public, anon, authenticated/)
  assert.match(migration, /member\.account_type = 'admin'/)
  assert.match(server, /cohortBucket\(`\$\{rollout\.rollout_salt\}:\$\{identity\.userId\}`\)/)
  assert.match(server, /admin\.from\('feature_rollouts'\).*\.in\('feature_key', keys\)/s)
  assert.match(publicRoute, /getFeatureFlagsServer\(Object\.values\(FEATURE_FLAGS\)\)/)
  assert.doesNotMatch(publicRoute, /rollout_percent|rollout_salt|audience/)
  assert.match(adminRoute, /new URL\(origin\)\.host !== request\.nextUrl\.host/)
  assert.match(adminRoute, /profile\?\.account_type !== 'admin'/)
  assert.match(manager, /value: 'percentage', label: 'Staged cohort'/)
  assert.match(manager, /type="range" min="1" max="99"/)
})

test('research workspaces support access-controlled collaboration, comments, presence, and attached data', async () => {
  const migration = await read('supabase/migrations/20260912201954_collaborative_research_workspaces.sql')
  const page = await read('src/app/workspace/page.tsx')
  const workspace = await read('src/components/research/ResearchWorkspaceClient.tsx')
  const collaboration = await read('src/components/research/WorkspaceCollaboration.tsx')
  assert.match(migration, /private\.can_view_research_workspace/)
  assert.match(migration, /private\.can_edit_research_workspace/)
  assert.match(migration, /role in \('viewer', 'editor'\)/)
  assert.match(migration, /alter table public\.research_workspace_comments enable row level security/)
  assert.match(migration, /user_id = \(select auth\.uid\(\)\) and private\.can_view_research_workspace/)
  assert.match(migration, /revoke all on public\.research_workspace_members, public\.research_workspace_comments, public\.research_workspace_presence from public, anon, authenticated/)
  assert.match(migration, /lower\(username\) = lower\(btrim\(p_username\)\)/)
  assert.match(migration, /revoke update on public\.research_workspaces from authenticated/)
  assert.match(migration, /save_research_workspace_board/)
  assert.match(migration, /coalesce\(cardinality\(p_watchlist_item_ids\), 0\) > 100/)
  assert.match(page, /sharedWatchlistIds/)
  assert.match(page, /admin\.from\('watchlist_items'\).*\.in\('id', sharedWatchlistIds\)/s)
  assert.match(workspace, /<WorkspaceCollaboration workspace=\{active\} userId=\{userId\} \/>/)
  assert.match(workspace, /workspace\.user_id !== userId \? ' · shared'/)
  assert.match(workspace, /rpc\('save_research_workspace_board'/)
  assert.match(collaboration, /research_workspace_presence/)
  assert.match(collaboration, /window\.setInterval\(heartbeat, 30_000\)/)
...
test('social composer and member previews use viewport-safe portaled surfaces', async () => {
  const floating = await read('src/components/ui/FloatingSurface.tsx')
  const gif = await read('src/components/social/GifPicker.tsx')
  const emoji = await read('src/components/social/EmojiPicker.tsx')
  const profile = await read('src/components/social/MentionProfileCard.tsx')
  const composer = await read('src/components/social/FeedComposer.tsx')
  const post = await read('src/components/social/PostCardClient.tsx')
  const styles = await read('src/app/globals.css')
  assert.match(floating, /createPortal\(/)
  assert.match(floating, /document\.body/)
  assert.match(floating, /window\.addEventListener\('scroll', place/)
  assert.match(floating, /window\.innerWidth - surfaceWidth - GUTTER/)
  assert.match(floating, /event\.key === 'Escape'/)
  assert.match(gif, /FloatingSurface open=\{open\}[\s\S]{0,240}mobileSheet/)
  assert.match(emoji, /FloatingSurface open=\{open\}[\s\S]{0,240}mobileSheet/)
  assert.match(composer, /FloatingSurface open=\{sportOpen\}/)
  assert.match(composer, /FloatingSurface open=\{visibilityOpen\}/)
  assert.match(profile, /onPointerEnter=\{keepOpen\}/)
  assert.match(profile, /align="start"/)
  assert.match(profile, /<UserBadges userId=\{profile\.id\}/)
  assert.match(profile, /ringStyle=\{profile\.avatar_ring_style\}/)
  assert.match(styles, /\.ss-floating-surface \{ position: fixed/)
  assert.match(styles, /\.ss-floating-surface\.is-mobile-sheet/)
