...
test('group conversations are private, bounded, realtime, and integrated with the message inbox', async () => {
  ...
  assert.ok(room.includes(".channel(`group-dm:${conversation.id}`)"))
  assert.ok(room.includes(".from('group_messages').insert"))
  assert.ok(room.includes("uploadMedia(file, 'messages')"))
